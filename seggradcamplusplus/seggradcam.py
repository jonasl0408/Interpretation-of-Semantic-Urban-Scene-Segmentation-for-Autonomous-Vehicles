...
class ClassRoIplusplus(SuperRoI):
...
class PixelRoIplusplus(SuperRoI):
...
class SegGradCAMplusplus:
    ...
    def featureMapsGradients(self):
        ...
        """
        img_tensor = tf.Variable(np.expand_dims(self.image, 0))
        ...
        heatmap_model = tf.keras.models.Model(
            [self.input_model.inputs], [self.input_model.get_layer(self.prop_to_layer).output,                      
            self.input_model.get_layer(self.prop_from_layer).output]
        )
        #plt.plot(self.input_model.get_layer(self.prop_to_layer).output)
        #plt.imshow()
        with tf.GradientTape() as tape:
            conv_output, y_c = heatmap_model(img_tensor)
            y_c = y_c[0,:,:,self.cls]
            #print("conv_output: ", conv_output.shape)
            #print("self.roi.roi: ", self.roi.roi[100,80])
        ...
        with tf.GradientTape(persistent=True) as tape2:
            ...
            #print("self.roi.roi: ", self.roi.roi[100,80])
                y_c = y_c * self.roi.roi  # Mask the region of interest
            #y_c = cv2.resize(y_c, (640,320), interpolation = cv2.INTER_AREA)
        grads_2 = tape2.gradient(y_c, conv_output)
        
        with tf.GradientTape(persistent=True) as tape3:
            tape3.watch(img_tensor)
            with tf.GradientTape(persistent=True) as tape2:
                tape2.watch(img_tensor)
                with tf.GradientTape() as tape:
                    tape.watch(img_tensor)
            ...
            #print("self.roi.roi: ", self.roi.roi[100,80])
                    y_c = y_c * self.roi.roi  # Mask the region of interest
            #y_c = cv2.resize(y_c, (640,320), interpolation = cv2.INTER_AREA)
        grads_3 = tape3.gradient(y_c, conv_output)
        print('grads_3.shape', grads_3.shape)
        self.A, self.grads_val = conv_output[0, :], grads[0, :, :, :]
        #print('self.A.shape:', self.A.shape)
        #print('self.grads_val:', self.grads_val.shape)
        #grads_power_2 = self.grads_val*self.grads_val
        #grads_power_3 = grads_power_2*self.grads_val
        grads_power_2 = grads_2[0, :, :, :]
        grads_power_3 = grads_3[0, :, :, :]
        #sum_activations = np.sum(self.A, axis=(1,2))
        
        sum_activations = np.sum(self.A, axis=(0,1))
        print(self.A.shape)
        print(sum_activations.shape)
        aij = grads_power_2 / (2 * grads_power_2 + sum_activations * grads_power_3)
        
        aij = np.where(self.grads_val != 0, aij, 0)
        print('aij.shape', aij.shape)
        weights = np.maximum(self.grads_val, 0) * aij
        print('weights.shape:', weights.shape)
        weights = np.sum(weights, axis=(0,1)).squeeze()
        #print('weights.shape:', weights.shape)
        self.grads_val_plus = weights
        #print("A: ", self.A.shape)
        #print("grads: ", self.grads_val.shape)
        return self.A, self.grads_val_plus
    ...
    def gradientWeights(self):
        ...
        #self.alpha_c = np.mean(self.grads_val_plus, axis=(0, 1))
        self.alpha_c = self.grads_val_plus
        #if self.abs_w:
