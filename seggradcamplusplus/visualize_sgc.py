...
#import .seggradcam
from .seggradcam import SegGradCAMplusplus,BiasRoI, SuperRoI, ClassRoIplusplus, PixelRoIplusplus
import numpy as np


class SegGradCAMplusplusplot(SegGradCAMplusplus):
    ...
    def explainBase(self, title1, title1bias, start_save_name, pixel=False):
        ...
        #fonts = int(fonts / 3)
        #scatter_size = int(scatter_size / 30)
        #plt.figure(figsize=(10 * scale_fig, 5 * scale_fig))
        plt.axis('off')
        plt.imshow(self.ximg/255., #vmin=0, vmax=1, 
        cmap=self.cmap_orig)
        # class contour
        ...
        #jet.set_label(label="Pixel Importance for Classifiction")
        #jet.ax.set_yticklabels([0, 0.2, 0.4, 0.6, 0.8, 1])
        if pixel:
            plt.scatter(self.roi.j, self.roi.i, color='white')  # j then i or i,j ?

        plt.savefig(os.path.join(self.outfolder,
                                 start_save_name + '_' + str(self.cls) + '_to_act' + self.prop_to_layer +'_'+
                                 self.timestr + ".png"))
    ...
    def baseGtPrediction(self, title2, start_save_name_tetra, pixel=False):
        ...
        plt.subplot(121)
        plt.imshow(self.ximg/255., vmin=0, vmax=1, cmap=self.cmap_orig)
